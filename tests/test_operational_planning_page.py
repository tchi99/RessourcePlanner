...
class OperationalPlanningPageTests(unittest.TestCase):
    ...
    def test_final_renderer_is_physically_defined_outside_versioned_modules(self) -> None:
        source = (APP / "operational_planning_renderer.py").read_text(encoding="utf-8")
        self.assertIn("def compose_operational_planning_renderer(", source)
        self.assertIn("def render_planning(owner: ui_module.PlannerUI)", source)
        self.assertIn("base_render(owner, weekly_stats_provider=ranked_stats)", source)
        self.assertNotIn("from . import v15", source)
    ...
    def test_v15_refinements_keeps_renderer_explicit_without_rewriting_planning_aliases(self) -> None:
        source = (APP / "v15_refinements.py").read_text(encoding="utf-8")
        self.assertIn("def _render_planning(", source)
        self.assertIn("def install_v15_refinements()", source)
        for token in (
            "PlannerUI.render_planning = _render_planning",
            "v13._render_operational_planning = _render_planning",
            "v15._render_operational_planning_v15 = _render_planning",
    ...
    def test_v15_installer_keeps_renderer_explicit_without_rewriting_legacy_aliases(self) -> None:
        source = (APP / "v15.py").read_text(encoding="utf-8")
        self.assertIn("def _render_operational_planning_v15(", source)
        self.assertIn("def install_v15_features()", source)
        for token in (
            "PlannerUI.render_planning = _render_operational_planning_v15",
