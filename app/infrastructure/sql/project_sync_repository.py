    ...
    def upsert_external_project(self, project: ExternalProjectRecord) -> str:
        ...
        if not external_id:
            raise ApplicationValidationError(
                "Un identifiant ERP est requis pour synchroniser un projet.",
                code="project_sync_external_id_required",
            )
        if not number:
            raise ApplicationValidationError(
                "Un numéro de projet est requis pour synchroniser un projet.",
                code="project_sync_number_required",
                context={"erp_external_id": external_id},
            )
        if not name:
            raise ApplicationValidationError(
        ...
        by_external = self._session.scalar(
            select(Project).where(Project.erp_external_id == external_id)
        )
        ...
        if row.erp_external_id and row.erp_external_id != external_id:
            raise ApplicationConflictError(
