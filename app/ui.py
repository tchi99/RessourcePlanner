...
class PlannerUI:
    def __init__(
        ...
        # Refreshables créés à partir de méthodes liées :
...
- Si plusieurs personnes modifient le même classeur simultanément, les conflits restent soumis aux limites de coédition/synchronisation d'Excel et OneDrive.
    ...
    def render_requests(self) -> None:
        demands = self.repo.demands()
        demand_rows = [
            self._demand_grid_row(d)
            for d in reversed(demands)
        ]
        ...
        with ui.row().classes(
            ...
            with ui.column().classes("gap-0"):
                ...
                ui.label(
                    "Clique une ligne pour afficher "
                    "les actions."
                ).classes("muted")
            ui.space()
            ui.button(
        ...
        def select_request(e: Any) -> None:
            ...
            data = args.get("data", {})
            self.selected_request = data.get(
                "NoDemande"
            )
    ...
    def _request_actions(self) -> None:
    ...
    def open_new_request_dialog(self) -> None:
        self.interaction_lock = True
        ...
        project_lookup = {}
        for p in projects:
            number = p.get("Numéro de Projet")
            if number in (None, ""):
                continue
            key = str(number)
            label = (
                p.get("Nom de référence")
                or p.get("Donneur d'ouvrage")
                or ""
            )
            project_options[key] = (
                f"{key} — {label}"
            )
            project_lookup[key] = p
        ...
        with ui.dialog() as dialog, ui.card().classes(
            ...
            def save_draft() -> None:
                try:
                    number = self.repo.create_demand(
                        build_payload(),
                        submit=False,
                    )
                    ui.notify(
    ...
    def open_planning_dialog(
        ...
        with ui.dialog() as dialog, ui.card().classes(
            ...
            def create_effort() -> None:
                if (
                    not tech.value
                    or not start.value
                ):
                    ui.notify(
                        "Technicien et date de début "
                        "sont requis.",
                        type="warning",
                    )
                    return
                ...
                try:
                    row = self.repo.append_effort(
                        {
                            ...
                            "Champs d'expertise": (
                                self.repo.expertise_for_competency(
                                    competency
                                )
                            ),
                            "Équipe/Technicien attitré": (
                    ...
                    self.repo.close_demand_as_planned(
                        str(
                            demand["NoDemande"]
                        ),
                        row,
    ...
    def _demand_grid_row(
        ...
        return {
            ...
            "Priorite": (
                demand.get("Priorite") or ""
            ),
            "Debut": PlannerUI._date_text(
                demand.get(
                    "DateDebutSouhaitee"
                )
            ),
            "Fin": PlannerUI._date_text(
                demand.get(
                    "DateFinSouhaitee"
                )
            ),
