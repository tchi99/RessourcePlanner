...
def _suspend_excel_ui(repo: ExcelRepository) -> dict[str, Any]:
    """Best-effort Excel UI/calculation suspension for a multi-write operation."""
    state: dict[str, Any] = {}
    try:
        app = repo._book().app
    except Exception:
...
def _install_repository_batching() -> None:
    ...
    def batch_update(self: ExcelRepository, label: str = "Excel batch"):
        """Group many xlwings writes behind one workbook save.

        Excel is still the source of truth. This is a write coalescing mechanism, not a
        rollback transaction: if an exception occurs after Excel cells were modified,
        the partial in-memory changes are saved once before the exception propagates.
        """
        with self._lock:
...
def _order_number(value: Any) -> float | None:
    return v17_refinements._order_number(value)


def _write_order_column(repo: ExcelRepository, updates: dict[str, Any]) -> bool:
    """Write the RessourcesMO order column in one contiguous xlwings call."""
    ...
    try:
        order_col = v16_refinements.RESOURCE_PROFILE_HEADERS.index(
            v17_refinements.RESOURCE_ORDER_FIELD
        ) + 1
    except ValueError:
        return False
    ...
    for excel_row in range(2, max_row + 1):
        ...
        current = _order_number(record.get(v17_refinements.RESOURCE_ORDER_FIELD))
...
def _manual_order_dialog_fast(self: ui_module.PlannerUI) -> None:
    ...
    v17_refinements._ensure_manual_order_column(self.repo)
...
def _install_resource_write_optimizations() -> None:
    if getattr(v17_refinements, "_v171_resource_writes_installed", False):
        return
...
def install_v171_performance() -> None:
    if getattr(ExcelRepository, "_v171_performance_installed", False):
        return

    # Install repository batching first: subsequent optimization wrappers rely on it.
    ...
    ExcelRepository._v171_performance_installed = True
